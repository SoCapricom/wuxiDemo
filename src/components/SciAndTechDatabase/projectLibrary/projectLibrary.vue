  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">科技计划项目</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'8')">重点研发计划</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'5')">科技重大专项与工程</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'2')">技术创新引导专项（基金）</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'4')">科技支撑计划</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  ...
                  <Row>
                    <Col span="12">
                    <FormItem label="计划类别" prop="typeSelect">
                      <Select v-model="serformItem.typeSelect" @on-change="selecterdto" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option  :key="index" v-for="(item,index) in registerTypeData" :value="item.id">{{ item.name }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                    <Col span="12">
                   <FormItem label="项目类别" prop="industrySelect">
                      <Select v-model="serformItem.industrySelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in industryTypeData" :value="item.id">{{ item.name }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                  </Row>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.projectName?item.projectName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.primaryUnit?item.primaryUnit:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.planTypeDto?item.planTypeDto.name:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.categoryDto?item.categoryDto.name:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.technicalField?item.technicalField:''}}</div></td>
            </tr>
  ...
  export default {
    ...
    data () {
      return {
        ...
        companyId:'',//存储公共id
        //标志位
        tabShowSign: 'ProjectLibrarySign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/projectLibrary/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求计划类别
      getRegisterTypeData(){
         postAjax('/projectType/queryListPage/' + 0 + '/' + this.pageSize,{pid:0})
          .then((response) => {
            if(response.data.code === 0){
       ...
       //请求项目类别
      selecterdto(){
         postAjax('/projectType/queryListPage/' + 0 + '/' + this.pageSize,{pid:this.serformItem.typeSelect})
          .then((response) => {
            if(response.data.code === 0){
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        let ifRegisterTypeName = this.serformItem.typeSelect === '' ;//计划类别
         let ifIndustryTypeName = this.serformItem.industrySelect === '' ;//项目类别
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.projectName = data //给json添加子对象
        };
        ...
        postAjax('/projectLibrary/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
