    ...
    <div id="topToolBar">
      <!--01-->
        <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
          <h3 class="fl PageTitle">新四板企业</h3>
            ...
            <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
              ...
              <div class="searchTerm fl">
                <div v-show="serShow" class="commonSearch pr">
                  ...
                  <ul class="scrollBar">
                    <li class="textoverflow" @click="getNewFourInformationList(0,10,$event,'创新板')">创新板</li>
                    <li class="textoverflow" @click="getNewFourInformationList(0,10,$event,'成长板')">成长板</li>
                    <li class="textoverflow" @click="getNewFourInformationList(0,10,$event,'科技板')">科技板</li>
                  </ul>
                </div>
                <div v-show="!serShow" class="customSearch pr">
                  ...
                  <Form ref="serformItem" :model="serformItem" :label-width="100">
                    <Row>
                      ...
                      <Col span="12">
                        <FormItem label="挂牌日期" prop="timeSelect">
                          <DatePicker v-model="serformItem.timeSelect" type="date" placeholder="请选择日期" style="width: 165px"></DatePicker>
                        </FormItem>
                      </Col>
                    </Row>
                    <Row>
                      <Col span="12">
                        <FormItem label="挂牌板块" prop="quotedPlate">
                          <Select v-model="serformItem.quotedPlate" style="width: 165px">
                            <Option  :key="index" v-for="(item,index) in typePlate" :value="item.name">{{ item.name }}</Option>
                    ...
                    <FormItem class="pa serBtns" style>
                      <Button type="primary" @click="getNewFourInformationList(0,10)">查询</Button>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr class="Table_List" :key="index" v-for="(item,index) in mainData" v-if="mainData.length>0">
              ...
              <td width="40%"><div class="textoverflow detailShow" @click.stop="showNewThreeDetail(item.companyCode)">{{item.companyName?item.companyName:''}}</div></td>
              <!-- <td width="20%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="25%"><div class="textoverflow">{{item.quotedPlate?item.quotedPlate:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.quotedTime?item.quotedTime:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/newFour/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
            if(response.data.code === 0){
              this.$Message.success('删除成功！');
               this.getNewFourInformationList(this.currentPage-1, this.pageSize);
              this.beforeSelect=true
              this.rightEnter=false;
              this.afterSelect=false;
              this.checkList = [];
      ...
      getTypeData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "listing_type"
        })
          .then((response) => {
            if(response.data.code === 0){

              this.typePlate = response.data.obj;
            }else {
      ...
      //请求主表格数据
      getNewFourInformationList(pageNum, pageSize,e,val) {
         this.$refs['pages'].currentPage = 1;
          this.valSearch='';
         let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//企业名称
        // let ifArea = this.serformItem.areaSelect === '' ;//所属区县
        let iftradeSelect = this.serformItem.quotedPlate === '' ;//上市板块
        ...
        //   }
        // };
        if(!iftradeSelect){//搜索值不为空—上市板块
          let data = this.serformItem.quotedPlate;
          if(data !== '全部'){//搜索值不全部
            param.quotedPlate =data
          }
        };
           if(val){
          this.valSearch=val;
          this.searchE=e;
          param.quotedPlate =val
         
        }
        if(!ifListedTime){//搜索值不为空—挂牌日期
          let date = this.serformItem.timeSelect;
          ...
          if(date !== ''){//搜索值不全部
            ...
            param.quotedTime = dataTime;
            // console.log(data);
          }
        };
        postAjax('/newFour/queryListPage/' + pageNum + '/' + pageSize,param )
          .then((response) => {
            if(response.data.code === 0){
    ...
    mounted() {
      this.getNewFourInformationList(0, this.pageSize);
