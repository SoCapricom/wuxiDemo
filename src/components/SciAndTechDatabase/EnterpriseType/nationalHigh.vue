  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">国家级级高企</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'196')">新认定</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'197')">重新认定</li>
                </ul>
              </div>
                  ...
                  <Row>
                    ...
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      ...
                      <Option :key="index" v-for="(item,index) in areaData" :value="item.regionCode">{{item.regionName}}</Option>
                       ...
                       <Col span="12">
                    <FormItem label="认证类型" prop="industrySelect">
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.companyName?item.companyName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.registeredDto?item.registeredDto.name:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.technicalDto?item.technicalDto.name:''}}</div></td>
              <!-- <td width="15%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.region_name:''}}</div></td> -->
               <td width="20%"><div class="textoverflow">{{item.typeDto?item.typeDto.name:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.thatYear?item.thatYear:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/nationalHigh/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求注册类型
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        let ifIndustryTypeName = this.serformItem.industrySelect === '' ;//认证类型
        ...
        //   if(data !== '全部'){//搜索值不全部
        //     param.registeredType =data
        //   }
        // };
    if(!ifIndustryTypeName){//搜索值不为空—认定类型
  ...
  if(val){
        ...
        postAjax('/nationalHigh/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){ 
