  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">新三板企业</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow" @click="getNewThreeInformationList(0,10,$event,'做市')">做市</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form ref="serformItem" :model="serformItem" :label-width="100">
                  ...
                  <Row>
                    <Col span="12">
                    <FormItem label="交易方式" prop="tradeSelect">
                      <Select v-model="serformItem.tradeSelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option value="做市"></Option>
                      </Select>
                    </FormItem>
                    </Col>
                  ...
                  <FormItem class="pa serBtns" style>
                    <Button type="primary" @click.stop="getNewThreeInformationList(0, 10)">查询</Button>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="30%"><div class="textoverflow detailShow" @click.stop="showNewThreeDetail(item.companyCode)">{{item.companyName}}</div></td>
              <!-- <td width="20%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="30%"><div class="textoverflow">{{item.dealStyle}}</div></td>
              <td width="30%"><div class="textoverflow">{{item.quotedTime}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/newTree/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
            if(response.data.code === 0){
              this.$Message.success('删除成功！');
               this.getNewThreeInformationList(this.currentPage-1, this.pageSize);
              this.beforeSelect=true
              this.rightEnter=false;
              this.afterSelect=false;
              this.checkList = [];
      ...
      //请求主表格数据
      getNewThreeInformationList(pageNum, pageSize,e,val) {
        this.$refs['pages'].currentPage = 1;
        this.valSearch='';
        let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//企业名称
        // let ifArea = this.serformItem.areaSelect === '' ;//所属区县
        let iftradeSelect = this.serformItem.tradeSelect === '' ;//上市板块
        ...
        if(!iftradeSelect){//搜索值不为空—上市板块
          let data = this.serformItem.tradeSelect;
          if(data !== '全部'){//搜索值不全部
            param.dealStyle =data
          }
        };
          if(val){
          this.valSearch=val;
        ...
        postAjax('/newTree/queryListPage/' + pageNum + '/' + pageSize,param)
          .then((response) => {
            if(response.data.code === 0){
    ...
    mounted() {
      this.getNewThreeInformationList(0, this.pageSize);
