          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getListedInformationList(0,10,$event,'73')">创业板</li>
                  <li class="textoverflow"  @click="getListedInformationList(0,10,$event,'74')">中小板</li>
                  <li class="textoverflow"  @click="getListedInformationList(0,10,$event,'75')">上交所</li>
                  <li class="textoverflow"  @click="getListedInformationList(0,10,$event,'76')">深交所主板</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                <i class="serArr block pa"></i>
                <!--表单列表 start-->
                <Form ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                    ...
                    <FormItem label="所属区县" prop="areaSelect">
                      ...
                      <Col span="12">
                      <FormItem label="上市时间" prop="timeSelect">
                  ...
                  <Row>
                    <Col span="12">
                      <FormItem label="证券代码/简称" prop="stoneCodeInput">
                        <Input v-model="serformItem.stoneCodeInput" style="width: 165px" placeholder="请输入证券代码/简称"></Input>
                      </FormItem>
                    </Col>
                    <Col span="12">
                      <FormItem label="上市板块" prop="listedPlate">
                        <Select v-model="serformItem.listedPlate" style="width: 165px">
                          <Option value="全部"></Option>
                          <Option :key="index" v-for="(item,index) in listedPlateData" :value="item.value">{{ item.name }}</Option>
                        </Select>
                      </FormItem>
                    </Col>
                  </Row>
                  ...
                  <FormItem class="pa serBtns" style>
                    <Button type="primary" @click.stop="getListedInformationList(0, 10)">查询</Button>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr class="Table_List" :key="index" v-for="(item,index) in mainData" v-if="mainData.length>0">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showNewThreeDetail(item.companyCode)">{{item.companyName}}</div></td>
              <!-- <td width="15%"><div class="textoverflow">{{item.areaInfo.regionName}}</div></td> -->
              <td width="20%"><div class="textoverflow">{{item.stoneCode}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.stoneName}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.listedPlateDto?item.listedPlateDto.name:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.listedTime}}</div></td>
            </tr>
  ...
  export default {
    ...
    data () {
      return {
        ...
        mainData: [],
        areaData:[],//区域数据
        listedPlateData: [],//上市板块数据
        companyId:'',//存储公共id
        //标志位
        tabShowSign: 'listedCompSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/listedCompanies/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
            if(response.data.code === 0){
              this.$Message.success('删除成功！');
               this.getListedInformationList(this.currentPage-1, this.pageSize);
              this.beforeSelect=true
              this.rightEnter=false;
              this.afterSelect=false;
              this.checkList = [];
      ...
      getListedPlateData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "list_plate"
        })
          .then((response) => {
            if(response.data.code === 0){
              this.listedPlateData = response.data.obj;
            }else {
              this.$Message.warning("上市板块数据请求异常");
            }
          })
          .catch(error => {
            console.log(error);
      ...
      //请求主表格数据
        getListedInformationList(pageNum, pageSize,e,val) {
          this.$refs['pages'].currentPage = 1;
          this.valSearch='';
        let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//企业名称
        let ifStoneCode = this.serformItem.stoneCodeInput === '' ;//证券代码
        // let ifArea = this.serformItem.areaSelect === '' ;//所属区县
        let ifListedPlate = this.serformItem.listedPlate === '' ;//上市板块
        let ifListedTime = this.serformItem.timeSelect === '' ;//上市时间
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.companyName = data //给json添加子对象
        };
        if(!ifStoneCode){//搜索值不为空—证券代码
          let data = this.serformItem.stoneCodeInput;
          param.stoneCode = data //给json添加子对象
        };
        // if(!ifArea){//搜索值不为空—所属区县
        //   let data = this.serformItem.areaSelect;
        //   if(data !== '全部'){//搜索值不全部
        //     param.areaInfo = {//给json添加子对象
        //       "regionName": data
        //     };
        //   }
        // };
        if(!ifListedPlate){//搜索值不为空—上市板块111111111
          let data = this.serformItem.listedPlate;
          if(data !== '全部'){//搜索值不全部
            param.listedPlateDto = {//给json添加子对象
              "value": data
            };
          }
        };
        ...
        if(!ifListedTime){//搜索值不为空—所属区县
          let data = this.serformItem.timeSelect;
          if(data !== ''){//搜索值不全部
            param.listedTime = data;
            console.log(data);
          }
        };
        postAjax('/listedCompanies/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
    ...
    mounted() {
      this.getListedInformationList(0, this.pageSize);
      this.getAreaData();//检索区域数据
      this.getListedPlateData();//检索上市板块数据
    },
    destroyed(){
