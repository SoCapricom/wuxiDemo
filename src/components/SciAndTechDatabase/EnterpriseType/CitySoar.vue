          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'194')">电子信息技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'186')">生物与新医药技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'187')">航空航天技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'188')">新材料技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'189')">高技术服务业</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'190')">新能源及节能技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'191')">资源与环境技术</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'192')">高新技术改造传统产业</li>
                </ul>
              </div>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.enterpriseName?item.enterpriseName:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.registrationDto?item.registrationDto.name:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.technicalDto?item.technicalDto.name:''}}</div></td>
              <!-- <td width="15%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="15%"><div class="textoverflow">{{item.transitTime?item.transitTime:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/municipalHigh/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求注册类型
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        //   if(data !== '全部'){//搜索值不全部
        //     param.registrationType =data
        //   }
        // };
    if(!ifIndustryTypeName){//搜索值不为空—技术领域
  ...
  if(val){
          this.valSearch=val;
          this.searchE=e;
          param.technicalField =val
         
        }
        postAjax('/municipalHigh/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){ 
