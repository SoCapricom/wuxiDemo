        ...
        <transition name="fade">
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchMenu fl">
              ...
              <a  class="block menuName transi" :class="{'active': serTitOn}" >自定义检索</a>
            </div>
            <!--检索导航 end-->
            <!--检索内容选择 start-->
            <div class="searchTerm fl">
              ...
              <div v-show="serShow" class="customSearch pr">
                <i class="serArr block pa"></i>
                <!--表单列表 start-->
                <Form ref="serformItem" :model="serformItem" :label-width="90">
                  <Row>
                    <Col span="12">
                    ...
                    <FormItem label="行业分类" prop="industrySelect">
                      <Select v-model="serformItem.industrySelect" style="width: 175px">
                  ...
                  <!-- <FormItem label="有无上缴税费" prop="taxRadio">
                    <RadioGroup v-model="serformItem.taxRadio">
                      ...
                      <Radio :key="index" v-for="(item,index) in payTaxesData" :label="item.value">{{ item.name }}</Radio>
                    </RadioGroup>
                  </FormItem> -->
                  <FormItem class="pa serBtns" style>
                    <!--<Button type="primary" @click="handleSubmit('serformItem')">查询</Button>-->
                    <Button type="primary" @click.stop="getEnterpriInformationList(0, 10)">查询</Button>
    ...
    <div @click.stop>
      <div id="mainTable">
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length > 0" class="Table_List" :class="{'Table_ListBg':index%2 != 0}" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.companyName}}</div></td>
              <td width="15%"><div v-if="item.industryCategoryDto" class="textoverflow">{{item.industryCategoryDto.name}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.registerTypeDto.name}}</div></td>
              <!-- <td width="13%"><div class="textoverflow">{{item.areaInfo.regionName}}</div></td> -->
              <td width="10%"><div class="textoverflow">{{item.industryOutputValue}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.totalIncome}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.totalPayment}}</div></td>
            </tr>
  ...
  export default {
    ...
    data () {
      return {
        ...
        industryTypeData:[],//行业分类
        payTaxesData: [],//上缴税费
        companyId:'',//存储公共id
        //标志位
        tabShowSign: 'techSmallCompSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
...
//      },
      ...
      Checkdelete(){
        // debugger
          postAjax('/techSmallCompany/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
            if(response.data.code === 0){
              this.$Message.success('删除成功！');
              this.getEnterpriInformationList(this.currentPage-1, this.pageSize);
              this.beforeSelect=true
              this.rightEnter=false;
              this.afterSelect=false;
              this.checkList = [];
      ...
      getPayTaxesDataData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "pay_taxes_status"
        })
          .then((response) => {
            if(response.data.code === 0){
              this.payTaxesData = response.data.obj;
            }else {
              this.$Message.warning("上缴税费数据请求异常");
            }
          })
          .catch(error => {
            console.log(error);
      ...
      //请求主表格数据
      getEnterpriInformationList(pageNum, pageSize) {
        ...
        // let ifPayTaxesName = this.serformItem.taxRadio === '' ;//上缴税费
        ...
        // if(!ifPayTaxesName){//搜索值不为空—上缴税费
        //   let data = this.serformItem.taxRadio;
        //   if(data !== '全部'){//搜索值不全部
        //     param.payTaxesStatusDto = {
        //       "value": data
        //     };
        //   }
        // };

        postAjax('/techSmallCompany/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
              this.isLoading = false;
              this.mainData = response.data.obj.content;
              this.totalNum = response.data.obj.totalPages;
            }else {
