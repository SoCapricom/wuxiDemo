  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div @click.stop>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.businessName	?item.businessName	:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.intellectualApplyDto?item.intellectualApplyDto.total:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.intellectualAuthorizeDto?item.intellectualAuthorizeDto.total:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.intellectualEffectiveDto?item.intellectualEffectiveDto.total:''}}</div></td>
      ...
      <!-- <div style="margin: 20px;overflow: hidden"> -->
          ...
          <Page :total="totalNum"  :page-size="pageSize" :current="1" @on-change="changePage"></Page>
        </div>
      <!-- </div> -->
    </div>
    <!--表格列表 end-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/intellectual/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize) {
        ...
        if(!ifEnterpName){//搜索值不为空—企业名称
          ...
          param.businessName = data //给json添加子对象
        };
        ...
        postAjax('/intellectual/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
      ...
      //    postAjax('/companyOrg/queryListPage/' + 0 + '/' + this.pageSize, {regTypeDto:{name:val}})
      //     .then((response) => {
      //       if(response.data.code === 0){
      //         this.isLoading = false;
      //         this.mainData = response.data.obj.content;
      ...
      changePage(pageNum){
        ...
        this.getBusinessInformationList(pageNum - 1, this.pageSize)
