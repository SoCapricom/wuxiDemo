        ...
        <transition name="fade">
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchMenu fl">
              ...
              <a href="javascript:;" class="block menuName transi mt15" :class="{'active': !serTitOn}" >自定义检索</a>
            </div>
            <!--检索导航 end-->
            <!--检索内容选择 start-->
            <div class="searchTerm fl">
              ...
              <div v-show="serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                      <FormItem label="纳税人名称" prop="nameInput">
                        <Input v-model="serformItem.nameInput" style="width: 165px" placeholder="请输入纳税人名称"></Input>
                      </FormItem>
                    </Col>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.taxpayerName?item.taxpayerName:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.identificationNumber?item.identificationNumber:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.vatBq?item.vatBq:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.vatJq?item.vatJq:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.taxBq?item.taxBq:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.taxJq?item.taxJq:''}}</div></td>
              <!-- <td width="10%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="10%"><div class="textoverflow">{{item.dataYear?item.dataYear:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/taxData/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize) {
        ...
        if(!ifEnterpName){//搜索值不为空—企业名称
          ...
          param.taxpayerName = data //给json添加子对象
        };
        ...
        postAjax('/taxData/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
