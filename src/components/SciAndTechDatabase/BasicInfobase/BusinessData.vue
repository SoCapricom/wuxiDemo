<template>
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    <!--顶部工具条 start-->
    <div id="topToolBar">
      <!--01-->
        ...
        <transition name="fade">
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchMenu fl">
              <a href="javascript:;" class="block menuName transi mt15" :class="{'active': serTitOn}" @click.stop="serShow = true;serTitOn=true">自定义检索</a>
            ...
            <div class="searchTerm fl">
              <!-- <div v-show="serShow" class="commonSearch pr">
                ...
                <!-- <ul class="scrollBar">
                  <li class="textoverflow"  @click="searchList($event,'国有企业')">国有企业</li>
                  <li class="textoverflow"  @click="searchList($event,'集体企业')">集体企业</li>
                  <li class="textoverflow"  @click="searchList($event,'有限责任公司')">有限责任公司</li>
                  <li class="textoverflow"  @click="searchList($event,'股份合作企业')">股份合作企业</li>
                  <li class="textoverflow"  @click="searchList($event,'股份有限公司')">股份有限公司</li>
                  <li class="textoverflow"  @click="searchList($event,'中外合资经营企业')">中外合资经营企业</li>
                  <li class="textoverflow"  @click="searchList($event,'外资企业')">外资企业</li>
                  <li class="textoverflow"  @click="searchList($event,'港、澳、台商独资经营企业')">港、澳、台商独资经营企业</li>
                </ul> -->
              <!-- </div> -->
              <div v-show="serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    ...
                    <Col span="12" class="codeMargin">
                      <FormItem label="组织机构代码/统一信用代码" prop="companyCodeInput">
                        <Input v-model="serformItem.companyCodeInput" style="width: 165px; margin-bottom: 0;" placeholder="请输入"></Input>
                      </FormItem>
                    </Col>
                  </Row>
                  <Row>
                    <Col span="12">
                    <FormItem label="行业分类" prop="industrySelect">
                      <Select v-model="serformItem.industrySelect" style="width: 165px">
                    ...
                    <!-- <FormItem label="注册类型" prop="typeSelect">
                      <Select v-model="serformItem.typeSelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in registerTypeData" :value="item.name"></Option>
                      </Select>
                    </FormItem> -->
                  <!-- <FormItem label="所属区县" prop="areaSelect">
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      <Option value="全部"></Option>
                      <Option :key="index" v-for="(item,index) in areaData" :value="item.regionName"></Option>
                    </Select>
                  </FormItem> -->
                    <!-- </Col> -->
                  </Row>
      ...
      <div class="clearfix editBox animated bounceInLeft" v-show="afterSelect">
        <ul class="fl clearfix f14">
          ...
          <li class="fl">
            <a href="javascript:;" class="f_6" @click="Checkdelete"><Icon size="16" type="trash-a" class="mr5"></Icon>删除</a>
          </li>
        </ul>
    ...
    <div @click.stop>
      <div id="mainTable">
        <table width="100%">
          <thead>
          <tr class="Table_Head">
            <th class="tc"><input v-model="checkAll" @change.stop="checkAllData()" type="checkbox" id="allAndNotAll"  /></th>
            <th>序号</th>
            <th>企业名称</th>
            <th>组织机构代码/统一信用代码</th>
            <th>行业类型</th>
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              <td width="5%" class="tc"><input v-model="checkList" :value="item.id" type="checkbox" @change="checkOne" ></td>
              <td width="5%"><div class="textoverflow">{{index+1}}</div></td>
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyName)">{{item.companyName?item.companyName:'无'}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.companyCode?item.companyCode:'无'}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.industryCategoryDto?item.industryCategoryDto.name:"无"}}</div></td>
              <!-- <td width="10%"><div class="textoverflow">{{item.areaInfoProvice?item.areaInfoProvice.regionName:'无'}}</div></td> -->
              <td width="10%"><div class="textoverflow">{{item.regNature?item.regNature:'无'}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.regTime?item.regTime:'无'}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.regMoney?item.regMoney:'无'}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.dictInfoDto?item.dictInfoDto.name:'无'}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/companyOrg/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getIndustryTypeData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "industry_category"
        })
          .then((response) => {
            if(response.data.code === 0){
              this.industryTypeData = response.data.obj;
            }else {
              this.$Message.warning("行业分类数据请求异常");
            }
          })
          .catch(error => {
            console.log(error);
      ...
      //请求主表格数据
      getBusinessInformationList(pageNum, pageSize) {
        this.$refs['pages'].currentPage = 1;
        let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//企业名称
        let ifCompanyCode = this.serformItem.companyCodeInput === '' ;//组织机构代码
        // let ifArea = this.serformItem.areaSelect === '' ;//所属区县
        // let ifRegisterTypeName = this.serformItem.typeSelect === '' ;//公司注册类型
        let ifIndustryTypeName = this.serformItem.industrySelect === '' ;//公司行业分类
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.companyName = data //给json添加子对象
        };
        if(!ifCompanyCode){//搜索值不为空—组织机构代码
          let data = this.serformItem.companyCodeInput;
          param.companyCode = data //给json添加子对象
        };
        // if(!ifArea){//搜索值不为空—所属区县
        //   let data = this.serformItem.areaSelect;
        //   if(data !== '全部'){//搜索值不全部
        //     param.areaInfoArea = {//给json添加子对象
        //       "regionName": data
        //     };
        //   }
        // };
        // if(!ifRegisterTypeName){//搜索值不为空—注册类型
        //   let data = this.serformItem.typeSelect;
        //   if(data !== '全部'){//搜索值不全部
        //     param.regTypeDto = {
        //       "name": data
        //     };
        //   }
        // };
        if(!ifIndustryTypeName){//搜索值不为空—行业分类
          let data = this.serformItem.industrySelect;
          if(data !== '全部'){//搜索值不全部
            param.industryCategoryDto = {
              "value": data
            };
          }
        };
        postAjax('/companyOrg/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
      ...
      changePage(pageNum){
        ...
        this.getSearchList(pageNum - 1, this.pageSize)
    ...
    mounted() {
      this.getSearchList(0, this.pageSize);
