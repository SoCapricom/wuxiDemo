  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">技术转移机构</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <!-- <li class="textoverflow"  @click="searchList($event,'申报示范机构')">申报示范机构</li>
                  <li class="textoverflow"  @click="searchList($event,'入选示范机构')">入选示范机构</li> -->
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'127')">服务性企业技术转移机构</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'128')">科研机构非转制科研机构</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'129')">科研机构转制科研机构</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'130')">生产性企业技术转移机构</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'131')">高等院校</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                      <FormItem label="服务机构名称" prop="nameInput">
                        <Input v-model="serformItem.nameInput" style="width: 165px" placeholder="请输入机构名称"></Input>
                      </FormItem>
                    </Col>
                     <Col span="12">
                    <FormItem label="服务机构类型" prop="typeSelect">
                  ...
                  <Row>
                    <Col span="12">
                    <FormItem label="是否申报示范单位" prop="areaSelect">
                    ...
                    <Col span="12">
                    <FormItem label="是否入选示范单位" prop="industrySelect">
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="30%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.serviceName?item.serviceName:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.legalInfo?item.legalInfo.name:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.serviceType?item.serviceType:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.isDeclarationAgency==0?'是':'否'}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.isDemonstrationFacility==0?'是':'否'}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/serviceAgencies/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
     ...
     getBusinessInformationList(pageNum, pageSize,e,val) {
        this.$refs['pages'].currentPage = 1;
           this.valSearch='';
        let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//服务名称
        let ifArea = this.serformItem.areaSelect === '' ;//是否申报示范单位
         let ifIndustryTypeName = this.serformItem.industrySelect === '' ;//服务机构类型
         let typeSelect = this.serformItem.typeSelect === '' ;//是否入选示范单位
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.serviceName = data //给json添加子对象
        };

        if(!ifArea){//搜索值不为空—是否申报示范单位
          let data = this.serformItem.areaSelect;
          if(data !== '全部'){//搜索值不全部
            param.isDeclarationAgency = data
          }
        };
         if(!ifIndustryTypeName){//搜索值不为空—服务机构类型
          let data = this.serformItem.industrySelect;
          if(data !== '全部'){//搜索值不全部
            param.isDemonstrationFacility	 = data
          }
        };
         if(!typeSelect){//搜索值不为空—是否入选示范单位
         let data = this.serformItem.typeSelect;
        //  alert(data.getFullYear())
        if(data !== '全部'){
         param.legalTypes = data;
        }
          ...
          this.searchE=e;
          // alert(val)
          param.legalTypes =val
         
        }
        postAjax('/serviceAgencies/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
    ...
    mounted() {
      ...
      this.getRegisterTypeData()//服务机构类型
    },
    destroyed(){
