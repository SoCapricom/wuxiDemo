          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'147')">技术服务</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'148')">技术开发</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'149')">技术转让</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'150')">技术咨询</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  ...
                  <Row>
                    <Col span="12">
                    <FormItem label="合同类别" prop="typeSelect">
                      <Select v-model="serformItem.typeSelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in ContractData" :value="item.value">{{ item.name }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                    <Col span="12">
                   <FormItem label="所属区域" prop="areaSelect">
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      <Option value="全部"></Option>
                      <Option :key="index" v-for="(item,index) in areaData" :value="item.id">{{item.regionName}}</Option>
                    </Select>
                  </FormItem>
                    </Col>
                  </Row>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="18%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.sellerName?item.sellerName:''}}</div></td>
              <td width="19%"><div class="textoverflow">{{item.projectName?item.projectName:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.technicalDto?item.technicalDto.name:''}}</div></td>
              <td width="10%"><div class="textoverflow">{{item.contractDto?item.contractDto.name:''}}</div></td>
              <td width="8%"><div class="textoverflow">{{item.turnover?item.turnover:''}}</div></td>
               <td width="9%"><div class="textoverflow">{{item.technologyTrading?item.technologyTrading:''}}</div></td>
                <td width="8%"><div class="textoverflow">{{item.tradingYear?item.tradingYear:''}}</div></td>
                 <td width="8%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/technologyTrading/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求技术领域
      ...
      getContractData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "type_contract"
        })
          .then((response) => {
            if(response.data.code === 0){
              this.ContractData = response.data.obj;
            }else {
              this.$Message.warning("注册类型数据请求异常");
            }
          })
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        let ifRegisterTypeName = this.serformItem.typeSelect === '' ;//合同类别
         let ifIndustryTypeName = this.serformItem.industrySelect === '' ;//技术领域
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.sellerName = data //给json添加子对象
        };
        ...
        if(!ifArea){//搜索值不为空—所属区县
          ...
          if(data !== '全部'){//搜索值不全部
            param.typeContract = data;
          }
        };
         if(!ifIndustryTypeName){//搜索值不为空—服务机构类型
          let data = this.serformItem.industrySelect;
          if(data !== '全部'){//搜索值不全部
            param.technicalField = data;
          }
        };
         if(val){
          this.valSearch=val;
          this.searchE=e;
          param.typeContract =val
         
        }
        postAjax('/technologyTrading/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
