  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">新型企业家</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl" >
              <div v-show="serShow" class="commonSearch pr" >
                ...
                <ul class="scrollBar">
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'2018')">2018年认定</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'2017')">2017年认定</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'2016')">2016年认定</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                    <FormItem label="姓名" prop="nameInput">
                  ...
                  <Row>
                    ...
                    <Col span="12">
                    <FormItem label="认定时间" prop="selectedType">
                      <DatePicker type="year" placeholder="选择年份" format="yyyy" v-model="serformItem.selectedType" style="width: 165px"></DatePicker>
                    </FormItem>
                    </Col>
                  </Row>
    ...
    <div @click.stop>
      <div id="mainTable">
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData" class="Table_List" :key="index" v-for="(item,index) in mainData">
              <td width="5%" class="tc"><input v-model="checkList" :value="item.iD" @change="checkOne" type="checkbox"></td>
              <td width="5%"><div class="textoverflow">{{index+1}}</div></td>
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.name?item.name:''}}</div></td>
              <td width="30%"><div class="textoverflow">{{item.companyName?item.companyName:''}}</div></td>
              <!-- <td width="20%"><div class="textoverflow">{{item.areaId?item.areaId:''}}</div></td> -->
              <td width="20%"><div class="textoverflow">{{item.contactsNum?item.contactsNum:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.year?item.year:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/newEntrepreneur/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      //请求主表格数据
       getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        //  alert(data.getFullYear())
          param.year = data.getFullYear() //给json添加子对象
        };
      if(val){
          this.valSearch=val;
          this.searchE=e;
        ...
        postAjax('/newEntrepreneur/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
