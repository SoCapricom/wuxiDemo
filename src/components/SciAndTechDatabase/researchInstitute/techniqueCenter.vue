  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">企业技术中心</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl" >
              <div v-show="serShow" class="commonSearch pr" >
                ...
                <ul class="scrollBar">
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'83')">国家级企业技术中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'88')">天津市企业技术中心</li>
                </ul>
              </div>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="30%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.projectName?item.companyName:''}}</div></td>
  ...
  export default {
    ...
    data () {
      return {
        ...
        areaList:'',//级别
        //标志位
        tabShowSign: 'techniqueCenterSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/comTechCenter/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        postAjax('/comTechCenter/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
