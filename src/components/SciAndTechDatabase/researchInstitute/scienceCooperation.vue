  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">国际科技合作基地</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl" >
              <div v-show="serShow" class="commonSearch pr" >
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'91')">国家国际创新园</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'92')">国家国际联合研究中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'93')">国家国际技术转移中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'94')">示范型国家国际科技合作基地</li>
                </ul>
              </div>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.baseDto?item.baseDto.name:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.companyName?item.companyName:''}}</div></td>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        areaList:'',//级别
        //标志位
        tabShowSign: 'scienceCooperationSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/nationalCooperationBase/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
 ...
 if(val){
          this.valSearch=val;
          this.searchE=e;
          param.projectName =val
         
        }
        postAjax('/nationalCooperationBase/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
