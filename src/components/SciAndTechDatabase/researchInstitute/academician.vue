  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">企业院士专家工作站</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl" >
              <div v-show="serShow" class="commonSearch pr" >
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'89')">市级</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'90')">区级</li>
                </ul>
              </div>
    ...
    <div @click.stop>
      <div id="mainTable">
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData" @click.stop="searchShow=false">
              ...
              <td width="20%"><div class="textoverflow">{{item.levelDto?item.levelDto.name:''}}</div></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/compExpertWorkstation/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
     ...
     getBusinessInformationList(pageNum, pageSize,e,val) {
      ...
      if(val){
        ...
        postAjax('/compExpertWorkstation/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
