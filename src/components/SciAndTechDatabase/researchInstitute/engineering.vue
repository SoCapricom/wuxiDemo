  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">工程研究中心</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl" >
              <div v-show="serShow" class="commonSearch pr" >
                ...
                <ul class="scrollBar">
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'80')">国家级工程技术研究中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'81')">国家级工程研究中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'86')">天津市工程中心</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'82')">国家地方联合工程实验室</li>
                </ul>
              </div>
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    ...
                    <FormItem label="名称" prop="nameInput">
                      <Input v-model="serformItem.nameInput" style="width: 165px" placeholder="实验室名称"></Input>
                    </FormItem>
                    </Col>
                    <Col span="12">
                    <FormItem label="依托单位" prop="operationName">
                      <Input v-model="serformItem.operationName" style="width: 165px" placeholder="请输入依托单位名称"></Input>
                    </FormItem>
                    </Col>
                  </Row>
                  <Row>
                    <Col span="12">
                    <FormItem label="所属区县" prop="selectedRank">
                      <Select v-model="serformItem.selectedRank" style="width: 175px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in areaList" :value="item.id">{{ item.regionName }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                    <Col span="12">
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="10%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.projectName?item.companyName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.typeDto?item.typeDto.name:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.companyName?item.companyName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.createDate?item.createDate:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/importantResearchCenter/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getRegisterTypeData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "important_institution_type"
        })
          .then((response) => {
            if(response.data.code === 0){
      ...
      getIndustryTypeData(){
        postAjax('/areaInfo/queryList', {
          ...
          .then((response) => {
            if(response.data.code === 0){
              this.areaList = response.data.obj;
            }else {
      ...
      //请求主表格数据
       getBusinessInformationList(pageNum, pageSize,e,val) {
          this.$refs['pages'].currentPage = 1;
           this.valSearch='';
        let param = {};
        let ifEnterpName = this.serformItem.nameInput === '' ;//姓名
        let ifArea = this.serformItem.operationName === '' ;//所属单位
        let ifRegisterTypeName = this.serformItem.selectedRank === '' ;//所属区县
        let selectedType = this.serformItem.selectedType === '' ;//认定时间
        if(!ifEnterpName){//搜索值不为空—姓名
          let data = this.serformItem.nameInput;
          param.name = data //给json添加子对象
        };
        if(!ifArea){//搜索值不为空—所属单位
         let data = this.serformItem.operationName;
          param.companyName = data //给json添加子对象
        };
        if(!ifRegisterTypeName){//搜索值不为空—所属区县
          let data = this.serformItem.selectedRank;
          if(data !== '全部'){//搜索值不全部
            param.areaId=data
          }
        };
        if(!selectedType){//搜索值不为空—认定时间
         let data = this.serformItem.selectedType;
        //  alert(data.getFullYear())
          param.type = data //给json添加子对象
        };
      ...
      if(val){
        ...
        postAjax('/importantResearchCenter/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
