          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'152')">企业法人</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'153')">事业法人</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'154')">社团法人</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'155')">民办非企业法人</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'156')">非独立法人</li>
                </ul>
              </div>
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    ...
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      ...
                      <Option :key="index" v-for="(item,index) in areaData" :value="item.regionCode">{{item.regionName}}</Option>
                    </Select>
                  </FormItem> -->
                        <FormItem label="法人类型" prop="legalPerson">
                      <Select v-model="serformItem.legalPerson" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in legalPersonData" :value="item.value">{{ item.name }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                  </Row>
                  <Row>
                    ...
                    <!-- <FormItem label="注册类型" prop="typeSelect">
                      <Select v-model="serformItem.typeSelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in registerTypeData" :value="item.value">{{item.name}}</Option>
                      </Select>
                    </FormItem> -->
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              <td width="5%" class="tc"><input v-model="checkList" :value="item.id" type="checkbox" @change="checkOne" name="childCheck"></td>
              <td width="5%"><div class="textoverflow">{{index+1}}</div></td>
              <td width="20%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.qydwxxmc26?item.qydwxxmc26:''}}</div></td>
              <!-- <td width="10%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="20%"><div class="textoverflow">{{item.xydlInfo?item.xydlInfo.name:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.qydjzclxInfo?item.qydjzclxInfo.name:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.frxzInfo?item.frxzInfo.name:''}}</div></td>
              <td width="15%"><div class="textoverflow">{{item.sjnf5?item.sjnf5:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/techTorch/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求注册类型
      ...
      getlegalPersonData(){
        postAjax('/dictInfo/queryList', {
          "groupCode": "h_legal_types"
        })
          .then((response) => {
            if(response.data.code === 0){
              this.legalPersonData = response.data.obj;
            }else {
              this.$Message.warning("行业分类数据请求异常");
            }
          })
      ...
      //请求主表格数据
       getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        let iflegalPerson = this.serformItem.legalPerson === '' ;//法人类型
        if(!ifEnterpName){//搜索值不为空—企业名称
          let data = this.serformItem.nameInput;
          param.qydwxxmc26 = data //给json添加子对象
        };
        ...
        //   if(data !== '全部'){//搜索值不全部
        //     param.qydjzclx249 =data
        //   }
        // };
    if(!ifIndustryTypeName){//搜索值不为空—行业分类
          let data = this.serformItem.industrySelect;
          if(data !== '全部'){//搜索值不全部
            param.xydl1wm255 =data
          }
        };
        if(!iflegalPerson){//搜索值不为空—法人分类
          let data = this.serformItem.legalPerson;
          if(data !== '全部'){//搜索值不全部
            param.frxz28 =data
          }
        };
if(val){
        ...
        postAjax('/techTorch/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){ 
...
if(val){
    ...
    mounted() {
      ...
      this.getIndustryTypeData()//行业分类
      // this.getRegisterTypeData()//注册类型
      this.getlegalPersonData()//法人类型
    },
    destroyed(){
