  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">创通票服务机构</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'4')">初创包</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'5')">新四板</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'6')">新三板</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'7')">知识产权</li>
                   <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'8')">高企包</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                      <FormItem label="机构名称" prop="nameInput">
                   ...
                   <FormItem label="所属区域" prop="areaSelect">
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      ...
                      <Col span="12">
                   <FormItem label="服务项目" prop="industrySelect">
                      <Select v-model="serformItem.industrySelect" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option :key="index" v-for="(item,index) in industryTypeData" :value="item.id">{{ item.serviceName }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
                  </Row>
                  <Row>
                    <Col span="12">
                    <FormItem label="服务包" prop="typeSelect">
                      <Select v-model="serformItem.typeSelect" @on-change="selecterdto" style="width: 165px">
                        <Option value="全部"></Option>
                        <Option  :key="index" v-for="(item,index) in registerTypeData" :value="item.id">{{ item.serviceName }}</Option>
                      </Select>
                    </FormItem>
                    </Col>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="40%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.agencyName?item.agencyName:''}}</div></td>
              <!-- <td width="15%"><div class="textoverflow">{{item.areaInfo?item.areaInfo.regionName:''}}</div></td> -->
              <td width="25%"><div class="textoverflow">{{item.servicePack?item.servicePack:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.serviceItems?item.serviceItems:''}}</div></td>
            </tr>
  ...
  export default {
    ...
    data () {
      return {
        ...
        companyId:'',//存储公共id
        //标志位
        tabShowSign: 'GenPassServiceSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/chuangtongInstitutions/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求服务包
      getRegisterTypeData(){
         postAjax('/packProject/queryListPage/' + 0 + '/' + this.pageSize,{pid:0})
          .then((response) => {
            if(response.data.code === 0){
              this.registerTypeData = response.data.obj.content;
              //  this.getProjectData();
            }else {
              this.$Message.warning("服务包数据请求异常");
            }
          })
          .catch(error => {
            console.log(error);
       ...
       //请求服务项目
      selecterdto(){
         postAjax('/packProject/queryListPage/' + 0 + '/' + this.pageSize,{pid:this.serformItem.typeSelect})
          .then((response) => {
            if(response.data.code === 0){
              this.industryTypeData = response.data.obj.content;
            }else {
              this.$Message.warning("服务包数据请求异常");
            }
          })
      ...
      //请求主表格数据
       getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        if(!ifEnterpName){//搜索值不为空—企业名称
          ...
          param.agencyName = data //给json添加子对象
        };
        ...
        // };
         ...
         if(!ifIndustryTypeName){//搜索值不为空—服务机构类型
          ...
          if(data !== '全部'){//搜索值不全部
            param.serviceItems = data
          }
        };
        if(val){
          this.valSearch=val;
          this.searchE=e;
          param.servicePack =val
         
        }
        postAjax('/chuangtongInstitutions/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
