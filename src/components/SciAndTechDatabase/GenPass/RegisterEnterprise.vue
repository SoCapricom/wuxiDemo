  ...
  <div style="background-color: #fff; height: 100%; min-width: 1000px" class="pr enterprisePage businessePage">
    ...
    <div id="topToolBar">
      ...
      <div class="clearfix pr animated bounceInRight" v-show="beforeSelect">
        <h3 class="fl PageTitle">创通票注册企业</h3>
          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <!-- <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar"> -->
                  <!-- <li class="textoverflow"  @click="searchList($event,'初创包')">初创包</li>
                  <li class="textoverflow"  @click="searchList($event,'新四板')">新四板</li>
                  <li class="textoverflow"  @click="searchList($event,'新三板')">新三板</li>
                  <li class="textoverflow"  @click="searchList($event,'知识产权')">知识产权</li>
                   <li class="textoverflow"  @click="searchList($event,'高企包')">高企包</li> -->
                <!-- </ul>
              </div> -->
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="40%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.businessName?item.businessName:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.lndustryClassification?item.lndustryClassification:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.registeredType?item.registeredType:''}}</div></td>
  ...
  export default {
    ...
    data () {
      return {
        ...
        companyId:'',//存储公共id
        //标志位
        tabShowSign: 'RegisterEnterpriseSign',
        //表格全选反选等
        checkAll: false,
        checkList: [],// 存放选中元素Id
        // 显示隐藏等效果的控制变量
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/registeredEnterprise/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize) {
        ...
        //   }
        // };
        postAjax('/registeredEnterprise/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
      ...
      searchList(e,val){
         ...
         postAjax('/registeredEnterprise/queryListPage/' + 0 + '/' + this.pageSize, {regTypeDto:{name:val}})
          .then((response) => {
            if(response.data.code === 0){
              this.isLoading = false;
              this.mainData = response.data.obj.content;
