          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'151')">ICT分会</li>
                  <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'99')">先进制造分会</li>
                  <li class="textoverflow" @click.stop="getBusinessInformationList(0, 10,$event,'100')">高新互联分会</li>
                  <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'101')">新三板同盟会</li>
                   <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'102')">人力资源</li>
                  <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'103')">知识产权专委会</li>
                  <li class="textoverflow"  @click.stop="getBusinessInformationList(0, 10,$event,'104')">文化传媒</li>
                  <li class="textoverflow" @click.stop="getBusinessInformationList(0, 10,$event,'105')">节能环保</li>
                </ul>
              </div>
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                     ...
                     <Col span="12">
                    <FormItem label="协会类型" prop="industrySelect">
                  ...
                  <FormItem class="pa serBtns" style>
                    <Button type="primary" @click.stop="getBusinessInformationList(0, 10,$event)">查询</Button>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.unitName?item.unitName:''}}</div></td>
              <td width="25%"><div class="textoverflow">{{item.industryClassification?item.industryClassification:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.registeredType?item.registeredType:''}}</div></td>
              <!-- <td width="10%"><div class="textoverflow">{{item.county?item.county:''}}</div></td> -->
              <td width="20%"><div class="textoverflow">{{item.associationInfo?item.associationInfo.name:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/highAssociation/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
   ...
   //请求协会类型
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        if(!ifEnterpName){//搜索值不为空—企业名称
          ...
          param.unitName = data //给json添加子对象
        };
        ...
        // };
         ...
         if(!ifIndustryTypeName){//搜索值不为空—服务机构类型
          ...
          if(data !== '全部'){//搜索值不全部
            param.associationType =data
          }
        };
        if(val){
          this.valSearch=val;
          this.searchE=e;
          param.associationType =val;
         
        }
        postAjax('/highAssociation/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
