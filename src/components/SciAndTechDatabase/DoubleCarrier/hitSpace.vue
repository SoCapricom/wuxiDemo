          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <!-- <li class="textoverflow" @click="searchList($event,'49')">国家级</li>
                  <li class="textoverflow" @click="searchList($event,'51')">市级</li>
                  <li class="textoverflow" @click="searchList($event,'52')">区级</li> -->
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'64')">投资促进型</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'68')">培训辅导型</li>
                  <li class="textoverflow" @click="getBusinessInformationList(0,10,$event,'59')">媒体延伸型</li>
                  <!-- <li class="textoverflow" @click="searchTypeList($event,'64')">专业服务型</li> -->
                  <!-- <li class="textoverflow" @click="searchTypeList($event,'64')">创客孵化型</li> -->
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                    <FormItem label="众创空间名称" prop="nameInput">
                      <Input v-model="serformItem.nameInput" style="width: 165px" placeholder="请输入众创空间名称"></Input>
                    </FormItem>
                    </Col>
                    <Col span="12">
                    <FormItem label="运营机构" prop="operationName">
                      <Input v-model="serformItem.operationName" style="width: 165px" placeholder="请输入机构名称"></Input>
                    </FormItem>
                    </Col>
                  </Row>
                  <Row>
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow">{{item.name}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.levelDto.name}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.typeDto.name}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/space/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        let ifIndustryTypeName = this.serformItem.selectedType === '' ;//类型
      ...
      if(!ifIndustryTypeName){//搜索值不为空—类型
  ...
  if(val){
        ...
        postAjax('/space/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
