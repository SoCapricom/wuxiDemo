            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  ...
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'52')">区级</li>
                  <!-- <li class="textoverflow" @click="searchTypeList($event,'53')">综合型</li>
                  <li class="textoverflow" @click="searchTypeList($event,'54')">专业型</li> -->
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    <Col span="12">
                    <FormItem label="孵化器名称" prop="nameInput">
        ...
        <table width="100%">
          ...
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              ...
              <td width="25%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.companyName?item.companyName:''}}</div></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/incubator/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      if(!ifIndustryTypeName){//搜索值不为空—类型
        ...
        postAjax('/incubator/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){
