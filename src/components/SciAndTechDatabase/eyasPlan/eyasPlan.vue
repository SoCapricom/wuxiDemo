          ...
          <div @click.stop="searchShow = true" id="searchPop" class="searchPop clearfix" v-show="searchShow">
            ...
            <div class="searchTerm fl">
              <div v-show="serShow" class="commonSearch pr">
                ...
                <ul class="scrollBar">
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'198')">电子信息</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'199')">生物与新医药</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'200')">航空航天</li>
                  <!-- <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'201')">新材料</li> -->
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'202')">服务业</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'203')">新能源</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'204')">环境方面</li>
                  <li class="textoverflow"  @click="getBusinessInformationList(0,10,$event,'205')">传统产业</li>
                </ul>
              </div>
              <div v-show="!serShow" class="customSearch pr">
                ...
                <Form class="scrollBar" ref="serformItem" :model="serformItem" :label-width="100">
                  <Row>
                    ...
                    <FormItem label="应用领域" prop="areaSelect">
                    <Select v-model="serformItem.areaSelect" style="width: 165px">
                      <Option value="全部"></Option>
                      <Option :key="index" v-for="(item,index) in areaData" :value="item.value">{{item.name}}</Option>
                    </Select>
                  </FormItem>
      ...
      <div id="mainTable">
        <table width="100%">
          ...
          <tr class="Table_Head">
            ...
            <th>企业名称</th>
            <th>主营产品应用领域</th>
            <th>数据类型(申报数据、年报数据)</th>
            <th>数据年份</th>
          </tr>
          </thead>
          <tbody>
            <tr v-if="mainData.length>0" class="Table_List" :key="index" v-for="(item,index) in mainData">
              <td width="5%" class="tc"><input v-model="checkList" :value="item.baseId" type="checkbox" @change="checkOne" name="childCheck"></td>
              <td width="5%"><div class="textoverflow">{{index+1}}</div></td>
              <td width="30%"><div class="textoverflow detailShow" @click.stop="showEnterpriDetail(item.companyCode)">{{item.userName?item.userName:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.projectinfo?item.projectinfo.technicalDto.name:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.technicalField?item.technicalField:''}}</div></td>
              <td width="20%"><div class="textoverflow">{{item.transitTime?item.transitTime:''}}</div></td>
            </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Checkdelete(){
        // debugger
          postAjax('/company/removeBatch?ids='+this.checkList.join(','))
          .then((response) => {
      ...
      getBusinessInformationList(pageNum, pageSize,e,val) {
        ...
        if(!ifEnterpName){//搜索值不为空—企业名称
          ...
          param.userName = data //给json添加子对象
        };
       ...
       if(val){
        ...
        postAjax('/company/queryListPage/' + pageNum + '/' + pageSize, param)
          .then((response) => {
            if(response.data.code === 0){ 
