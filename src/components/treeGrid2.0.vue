    ...
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
            ...
            <tbody class="triggerStyle">
                <tr v-for="(item,index) in initItems" :key="item.deptId" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column) class="pr">
                        <label v-if="column.type === 'selection'">
                            <input type="checkbox" :value="item.deptId" v-model="checkGroup" @click="handleCheckClick(item,$event,index)">
                        </label>
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type" class="pa triggerStyle_label">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-chevron-right':!item.expanded,'ivu-icon-chevron-down':item.expanded }"></i>
    ...
    methods: {
      ...
      iconRow() {
        ...
        handleCheckClick(data, event, index){
            ...
            if(arr){
                if(data.isChecked){
                    this.checkGroup.push(data.deptId);
                    for (let i=0; i<arr.length; i++){
                        this.checkGroup.push(arr[i].deptId)
                    }
                }else {
                    for (var i=0; i<this.checkGroup.length; i++){
                        if(this.checkGroup[i] == data.deptId){
                            this.checkGroup.splice(i,1)
                        }
                        for (var j=0; j<arr.length; j++){
                            if(this.checkGroup[i] == arr[j].deptId){
                                this.checkGroup.splice(i,1);
                            }
                        }
                    }
